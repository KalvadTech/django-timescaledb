...
class TimescaleQuerySet(models.QuerySet):
    def time_bucket(self, field: str, interval: str, annotations: Dict = None):
        ...
        if annotations:
            return (
                self.values(bucket=TimeBucket(field, interval))
                .order_by("-bucket")
    ...
    def time_bucket_ng(self, field: str, interval: str, annotations: Dict = None):
        ...
        if annotations:
            return (
                self.values(bucket=TimeBucketNG(field, interval))
                .order_by("-bucket")
                .annotate(**annotations)
            )
    ...
    def time_bucket_gapfill(
        ...
        """
        return self.values(
            bucket=TimeBucketGapFill(field, interval, start, end, datapoints)
        )
    ...
    def histogram(
        ...
        """
        return self.values(
            histogram=Histogram(field, min_value, max_value, num_of_buckets)
        )
