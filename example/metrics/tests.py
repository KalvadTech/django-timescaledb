...
class TimescaleDBTests(TestCase):
    ...
    def test_time_bucket_ng(self):
        ...
        Metric.objects.create(
            time=timestamp - relativedelta(months=1, days=15), temperature=14
        )
        Metric.objects.create(
            time=timestamp - relativedelta(months=1, days=10), temperature=12
        )

        # get all metrics, monthly aggregated
        metrics = Metric.timescale.time_bucket_ng("time", "1 month").annotate(
            Avg("temperature")
        )
